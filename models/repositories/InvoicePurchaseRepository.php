...
class InvoicePurchaseRepository
{
    ...
    public function getAll( $limit = 0, $order="addDate") {
        ...
        $query = "SELECT `id`, `invoiceNumber`, `addDate`,k.`vatID`,`name`, `amountNet`, `amountTax`, `amountGross`, `amountNetCurrencyValue`, `amountNetCurrency` FROM `invoicepurchase` as `f` INNER JOIN `contractor` as `k` on k.vatID=f.vatID ORDER BY $order DESC".($limit != 0 ? " LIMIT $limit" : "");
        ...
        $result = $sth->fetchAll(PDO::FETCH_CLASS, "InvoicePurchase");
        return $result;
    }
    ...
    public function getNumberOfRecords() {
        ...
        $pdo = new PDO($config['dsn'], $config['login'], $config['password']);
        $sth = $pdo->query("SELECT COUNT(*) as `count` from `invoicepurchase` ");
    ...
    public static function findBy($conditions ,$order="addDate") {
        ...
        $pdo = new PDO($config['dsn'], $config['login'], $config['password']);
        $query = "SELECT `id`, `invoiceNumber`, `addDate`, `k`.`vatID`,`name`, `amountNet`, `amountTax`, `amountGross`, `amountNetCurrencyValue`, `amountNetCurrency` FROM `invoicepurchase` as `f` INNER JOIN `contractor` as `k` on k.vatID=f.vatID WHERE "
            .$condition
            ." ORDER BY $order DESC";
        $sth = $pdo->prepare($query);
